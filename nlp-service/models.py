...
class NLPProcessResponse(BaseModel):
    ...
    context_updates: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Suggested context updates"
    )
    
    requires_escalation: bool = Field(False, description="Whether this requires human escalation")
    
    confidence_overall: float = Field(..., description="Overall confidence of the analysis")
...
class EntityExtractionResponse(BaseModel):
    ...
    entities: List[Entity] = Field(default_factory=list, description="Extracted entities")
    text_chunks: Optional[List[str]] = Field(None, description="Text chunks with annotations")
...
class HealthCheckResponse(BaseModel):
    ...
    models_loaded: Dict[str, bool] = Field(default_factory=dict, description="Status of loaded models")
...
class OllamaResponseRequest(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        None,
        description="DEPRECATED: Ollama doesn't support max_tokens. Ignored."
    )
    user_id: Optional[str] = Field(
        None,
...
class OllamaResponseResponse(BaseModel):
    ...
    generation_time_ms: float = Field(..., description="Time taken to generate response")
