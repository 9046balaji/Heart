...
class UserDeliveryPreference:
    ...
    user_id: str
    enabled: bool = True
    channels: List[DeliveryChannel] = field(default_factory=lambda: [DeliveryChannel.WHATSAPP])
...
class WeeklySummaryJob:
    ...
    async def execute(self, user_ids: Optional[List[str]] = None) -> List[DeliveryResult]:
        """
        ...
        logger.info(f"Weekly summary job completed. Processed {len(results)} deliveries")
        return results
    
    async def _process_user(self, user_pref: UserDeliveryPreference) -> List[DeliveryResult]:
        ...
        if weekly_data.data_completeness < 0.1:
            ...
            results.append(DeliveryResult(
                user_id=user_pref.user_id,
                channel=user_pref.channels[0] if user_pref.channels else DeliveryChannel.WHATSAPP,
    ...
    async def _get_user_preference(self, user_id: str) -> Optional[UserDeliveryPreference]:
    ...
    async def _update_last_delivery(
        ...
        try:
            successful = next(
                (r for r in results if r.status == DeliveryStatus.SENT), 
                None
            )
