...
class TaskExecutor:
    ...
    async def execute_plan(
        ...
        for group_idx, group in enumerate(groups):
            logger.debug(f"Executing group {group_idx + 1}/{len(groups)}: {len(group)} tasks")
            
            # Execute tasks in parallel within group
            group_results = await asyncio.gather(
                *[
                    self._execute_task(task, results, context or {})
                    for task in group
                ],
        ...
        failed = sum(1 for r in task_results if r.status in [TaskStatus.FAILED, TaskStatus.TIMEOUT])
    ...
    async def _execute_task(
        ...
        async with self._semaphore:
            try:
                ...
                execution_time = (time.perf_counter() - start_time) * 1000
                logger.warning(f"Task {task.task_id} timed out after {execution_time:.0f}ms")
                return TaskResult(
                    task_id=task.task_id,
                    agent_type=task.agent_type.value,
