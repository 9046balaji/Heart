...
class EmailNotificationService:
    ...
    def __init__(
        ...
        self.from_email = from_email or os.getenv('EMAIL_FROM', 'noreply@healthcare-chatbot.com')
        ...
        if self.mock_mode:
            ...
            logger.info(f"Email service initialized: {self.from_email} via {self.smtp_server}:{self.smtp_port}")
...
class SMSNotificationService:
    ...
    def __init__(
        ...
        self.from_number = from_number or os.getenv('TWILIO_PHONE_NUMBER', '+1234567890')
...
class PushNotificationService:
    ...
    def __init__(
        ...
        Args:
            ...
            mock_mode: Skip actual sending (for testing)
        """
        self.credentials_file = credentials_file or os.getenv('FIREBASE_CREDENTIALS_FILE', '')
        ...
        if FIREBASE_AVAILABLE and self.credentials_file and not mock_mode:
            try:
                ...
                logger.info("Push notification service initialized with Firebase Admin SDK")
            except Exception as e:
        ...
        if not self.use_admin_sdk and self.server_key and not mock_mode:
            if REQUESTS_AVAILABLE:
                logger.info("Push notification service initialized with legacy HTTP API")
            else:
    ...
    def send_push(
        ...
        try:
            ...
            if self.server_key and REQUESTS_AVAILABLE:
                ...
                if response.status_code == 200:
                    ...
                    if result.get('success', 0) > 0:
                        logger.info(f"Push sent via legacy API to {device_token[:20]}...")
                        return {"success": True, "response": result}
                    else:
                        logger.error(f"FCM push failed: {result}")
                        return {"success": False, "error": result.get('results', [{}])[0].get('error', 'Unknown')}
                else:
            ...
            logger.error(f"Failed to send push notification to {device_token[:20]}...: {e}")
            return {"success": False, "error": str(e)}
