...
class CalendarEvent(BaseModel):
    ...
    duration_minutes: int = Field(..., ge=15, le=480, description="Event duration in minutes")
    ...
    # Sync tracking
    synced_at: datetime = Field(default_factory=datetime.now, description="Last sync time")
    ...
    def validate_duration(self):
        ...
        calculated_duration = int((self.end_time - self.start_time).total_seconds() / 60)
...
class CalendarSync(BaseModel):
    ...
    started_at: datetime = Field(default_factory=datetime.now, description="Sync start time")
    ...
    sync_errors: List[Dict[str, str]] = Field(
        ...
        description="List of individual sync errors"
    )
...
class Reminder(BaseModel):
    ...
    minutes_before: int = Field(..., ge=5, le=10080, description="Minutes before appointment")
    ...
    recipient_device_id: Optional[str] = Field(None, description="Push notification device ID")
    ...
    def validate_recipient(self):
        ...
        if self.notification_type == NotificationType.EMAIL and not self.recipient_email:
        ...
        if self.notification_type == NotificationType.PUSH and not self.recipient_device_id:
...
class SyncAppointmentsRequest(BaseModel):
    ...
    user_email: str = Field(..., description="Calendar user email")
    appointment_ids: Optional[List[str]] = Field(None, description="Specific appointments to sync")
...
class ReminderScheduleResponse(BaseModel):
    ...
    failed_count: int = Field(..., description="Failed count")
    next_reminder_time: Optional[datetime] = Field(None, description="Next reminder time")
