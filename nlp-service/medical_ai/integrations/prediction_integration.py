...
class PredictionIntegrationService:
    ...
    RECOMMENDED_FEATURES = [
        "hdl", "ldl", "fasting_blood_sugar",
        "blood_pressure_systolic", "resting_heart_rate"
    ]
    
    # All supported features
    ALL_FEATURES = [
        "age", "sex", "cholesterol", "hdl", "ldl", "triglycerides",
        "fasting_blood_sugar", "hba1c", "creatinine",
    ...
    def extract_prediction_features(
        ...
        features = {
            ...
            "sex": self._normalize_sex(patient_profile.get("sex") or patient_profile.get("gender")),
    ...
    def validate_prediction_input(
        ...
        for feature in self.ALL_FEATURES:
            if feature not in self.REQUIRED_FEATURES and feature not in self.RECOMMENDED_FEATURES:
                value = getattr(input_data, feature, None)
        ...
        return {
            ...
            "message": (
                "Ready for prediction" if is_valid 
    ...
    async def run_prediction(
        ...
        if not validation["is_valid"]:
            raise ValueError(
                f"Invalid prediction input: {validation['message']}"
            )
                    ...
                    if response.status == 200:
                        ...
                        if self.audit_service:
                            self.audit_service.log_event({
                                ...
                                "risk_score": risk_score,
                                "features_count": len(validation["present_features"])
                            })
                        
                        return result
                    else:
                        error_text = await response.text()
                        logger.error(f"Prediction API error: {response.status} - {error_text}")
    ...
    def _generate_mock_prediction(
        ...
        if input_data.ldl and input_data.ldl > 130:
            risk_factors += 1
        if input_data.blood_pressure_systolic and input_data.blood_pressure_systolic > 140:
            risk_factors += 1
