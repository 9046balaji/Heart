...
class PatientTimelineService:
    ...
    def _create_lab_events(
        ...
        lab_event.data = {"test_count": len(test_results), "results_summary": []}
        ...
        if abnormal_count > 0:
            ...
            lab_event.description = (
                f"Lab results with {abnormal_count} abnormal value(s)"
            )
    ...
    def _create_discharge_events(
        ...
        discharge_event = TimelineEvent(
            ...
            description=extracted_data.get(
                "discharge_diagnosis", "Discharged from hospital"
            ),
            source="document",
            source_id=document_id,
            data={
    ...
    def get_timeline_summary(self, user_id: str, days: int = 7) -> TimelineSummary:
        """
    ...
    def _assess_vital_importance(
        ...
        if vital_type_lower in thresholds:
            ...
            ) or value >= limits.get("critical_high", float("inf")):
                return EventImportance.CRITICAL
            elif value < limits.get("low", float("-inf")) or value > limits.get(
                "high", float("inf")
            ):
