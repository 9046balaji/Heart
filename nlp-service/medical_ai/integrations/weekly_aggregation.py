...
class WeeklyAggregationService:
    ...
    def generate_weekly_summary(
        ...
        logger.info(
            f"Generating weekly summary for {user_id}: {start_date} to {end_date}"
        )
    ...
    def _aggregate_health_stats(
        ...
        if self.timeline:
            ...
            if steps_by_day:
                ...
                stats.avg_steps_per_day = round(
                    stats.total_steps / len(steps_by_day), 0
                )
                stats.steps_goal_met_days = sum(
                    1
                    for steps in steps_by_day.values()
    ...
    def _aggregate_medications(
        ...
        if self.timeline:
            ...
            stats.total_doses_scheduled = (
                stats.total_doses_taken + stats.total_doses_missed
            )
    ...
    def _aggregate_exercise(
        ...
        if self.timeline:
            ...
            stats.goal_completion_percent = round(
                min(100, stats.total_active_minutes / stats.active_minutes_goal * 100),
                1,
            )
    ...
    def _get_latest_risk(self, user_id: str) -> tuple:
        ...
        if self.timeline:
            ...
            events = self.timeline.get_timeline(
                user_id, event_types=[TimelineEventType.PREDICTION_RUN], limit=1
            )
    ...
    def _generate_personalized_tip(
        ...
        # Default tip
        if not tips:
            tips.append("Keep up the great work on your health journey! 🌟")
