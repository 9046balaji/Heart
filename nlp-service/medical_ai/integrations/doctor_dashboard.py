...
class DoctorDashboardService:
    ...
    async def get_patient_overview(
        self, patient_id: str, physician_id: str, physician_role: PhysicianRole
    ) -> PatientOverview:
        """
    ...
    async def get_document_history(
        ...
        if self.timeline:
            ...
            events = self.timeline.get_timeline(
                patient_id, event_types=event_types, limit=limit
            )
    ...
    async def get_medication_reconciliation(
        self, patient_id: str, physician_id: str
    ) -> Dict[str, Any]:
        """
    ...
    def _get_patient_profile(self, patient_id: str) -> Dict[str, Any]:
        ...
        return self._patient_data.get(
            patient_id, {"name": "Patient", "age": 45, "gender": "Unknown"}
        )

    async def _check_physician_consent(
        self, patient_id: str, physician_id: str, role: PhysicianRole
    ) -> bool:
    ...
    async def _get_recent_labs(self, patient_id: str) -> List[Dict[str, Any]]:
        ...
        if self.timeline:
            ...
            events = self.timeline.get_timeline(
                patient_id, event_types=[TimelineEventType.LAB_RESULT], limit=5
            )
            ...
            return [
                {
                    ...
                    "abnormal_count": sum(
                        1
                        for r in e.data.get("results_summary", [])
                        if r.get("abnormal")
                    ),
                }
                for e in events
    ...
    async def _get_current_medications(self, patient_id: str) -> List[Dict[str, Any]]:
        ...
        if self.timeline:
            ...
            events = self.timeline.get_timeline(
                patient_id, event_types=[TimelineEventType.PRESCRIPTION], limit=20
            )
    ...
    async def _get_risk_assessment(self, patient_id: str) -> Dict[str, Any]:
        ...
        if self.timeline:
            ...
            events = self.timeline.get_timeline(
                patient_id, event_types=[TimelineEventType.PREDICTION_RUN], limit=1
            )
    ...
    def _generate_alerts(
        self, vitals: Dict[str, Any], labs: List[Dict[str, Any]], risk: Dict[str, Any]
    ) -> List[str]:
