...
class ChatbotDocumentContextService:
    ...
    TOPIC_KEYWORDS = {
        "lab": ["lab", "test", "result", "blood", "cholesterol", "glucose", "hdl", "ldl", 
                "hemoglobin", "creatinine", "hba1c", "triglycerides", "panel"],
        "medication": ["medication", "medicine", "drug", "prescription", "dose", "dosage",
                       "pill", "tablet", "capsule", "taking", "prescribed"],
        "vitals": ["heart rate", "blood pressure", "bp", "pulse", "temperature", "oxygen",
                   "vital", "vitals", "reading"],
        "diagnosis": ["diagnosis", "condition", "disease", "disorder", "illness", "diagnosed"],
        "hospital": ["hospital", "admission", "discharge", "inpatient", "emergency", "er"],
        "risk": ["risk", "prediction", "chance", "likelihood", "heart disease", "cardiovascular"],
    }
    ...
    def get_relevant_context(
        ...
        for doc_id, doc_data, relevance in top_docs:
            ...
            sources.append(DocumentContext(
                document_id=doc_id,
    ...
    def _extract_query_keywords(self, query: str) -> List[str]:
        ...
        stop_words = {"what", "when", "where", "how", "why", "the", "and", "for", 
                      "are", "was", "were", "been", "have", "has", "had", "does",
                      "did", "can", "could", "would", "should", "may", "might",
                      "will", "shall", "about", "from", "with", "this", "that",
                      "these", "those", "your", "you", "my", "me", "tell", "show"}
    ...
    def _extract_query_relevant_data(
        ...
        if "medications" in extracted_data:
            ...
            for med in extracted_data["medications"]:
                med_name = med.get("name", "").lower()
                if any(kw in med_name for kw in query_keywords) or \
                   "medication" in query_lower or "medicine" in query_lower:
    ...
    def _is_medical_term(self, word: str) -> bool:
        ...
        medical_suffixes = ["emia", "itis", "osis", "ectomy", "plasty", "gram", 
                           "graph", "scopy", "logy", "pathy"]
        medical_prefixes = ["hyper", "hypo", "cardio", "hemo", "hepat", "nephro",
                           "neuro", "gastro", "pulmo"]
