    ...
    async def initialize(self):
        ...
        if not self.use_mock:
            try:
                ...
                logger.warning(
                    f"Failed to initialize Gateway for Food Recognition: {e}"
                )
                self.use_mock = True
...
Format your response with clear sections for each food item and totals."""
    ...
    def _parse_response(
        ...
        protein_match = re.search(
            r"protein[:\s]*(\d+(?:\.\d+)?)\s*g", response_text.lower()
        )
        carbs_match = re.search(
            r"carb(?:ohydrate)?s?[:\s]*(\d+(?:\.\d+)?)\s*g", response_text.lower()
        )
        fat_match = re.search(
            r"(?<!saturated\s)fat[:\s]*(\d+(?:\.\d+)?)\s*g", response_text.lower()
        )
        ...
        if any(
            ...
            for word in ["excellent", "very healthy", "heart-healthy"]
        ):
            health_score = 9.0
        elif any(
            word in response_lower for word in ["healthy", "good choice", "nutritious"]
        ):
            health_score = 7.5
        elif any(
            word in response_lower
            for word in ["unhealthy", "high sodium", "fried", "processed"]
        ):
            health_score = 4.0
    ...
    def _generate_recommendations(
        ...
        if nutrition.fiber_g and nutrition.fiber_g < 5:
            recs.append(
                "Consider adding more fiber-rich foods like vegetables or whole grains"
            )
    ...
    def _generate_warnings(self, nutrition: NutritionInfo) -> List[str]:
        ...
        if nutrition.sodium_mg and nutrition.sodium_mg > 800:
            warnings.append(
                f"⚠️ High sodium ({nutrition.sodium_mg:.0f}mg) - limit daily intake"
            )
