...
class ModelVersionManager:
    ...
    def _compute_model_hash(self, model_path: str) -> str:
        ...
        try:
            ...
            return hashlib.md5(str(model_path).encode()).hexdigest()
...
class PerformanceMonitor:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_avg_latency_ms": round(sum(recent_latencies) / len(recent_latencies), 2),
            ...
            "fallback_rate": round(self.total_fallbacks / self.total_predictions * 100, 2),
...
class SHAPExplainer:
    ...
    FEATURE_NAMES = [
        ...
        "fasting_bs", "rest_ecg", "max_hr", "exercise_induced_angina",
        "oldpeak", "st_slope"
    ]
    ...
    def explain_prediction(
        ...
        try:
            ...
            return {
                ...
                "top_risk_factors": [
                    f for f, v in sorted_features[:3] 
                    if v["shap_value"] > 0
                ],
...
class MLRiskAssessor:
    ...
    MODEL_PATHS = [
        ...
        Path(__file__).parent.parent / "cardio-ai-assistant" / "backend" / "models" / "stacking_ensemble_model.joblib",
        Path(__file__).parent.parent / "cardio-ai-assistant" / "backend" / "models" / "stacking_heart_disease_model.joblib",
        Path(__file__).parent.parent / "cardio-ai-assistant" / "backend" / "models" / "fitted_mlp_model.joblib",
    ]
    ...
    def _load_model(self):
        ...
        if not self.model_loaded:
            logger.warning("No pre-trained model found. ML risk assessment will use fallback methods.")
    ...
    def assess_risk(
        ...
        input_hash = hashlib.md5(str(sorted(metrics.items())).encode()).hexdigest()[:8]
        used_fallback = False
        error_msg = None
        ...
        try:
            if not self.model_loaded:
                ...
                risk_level, confidence = self._interpret_prediction(prediction, probability)
            ...
            self.monitor.record_prediction(PredictionRecord(
    ...
    def _interpret_prediction(self, prediction: float, probability: Optional[np.ndarray]) -> tuple:
        """
        ...
        if prediction == 0:
            ...
            risk_level = "MODERATE" if probability is None or probability[1] < 0.7 else "HIGH"
        else:
            risk_level = "MODERATE"
    ...
    def _generate_rule_explanation(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        age = metrics.get("age", 50)
        if age > 65:
            risk_factors.append(f"Age ({age}) is above 65, increasing cardiovascular risk")
        elif age < 45:
        ...
        if chol > 240:
            risk_factors.append(f"High cholesterol ({chol} mg/dL) - above recommended level")
        elif chol < 200:
            protective_factors.append(f"Healthy cholesterol level ({chol} mg/dL)")
        
        hr = metrics.get("max_hr", 150)
        if hr < 100:
            risk_factors.append(f"Low maximum heart rate ({hr}) may indicate reduced cardiac function")
