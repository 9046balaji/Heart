...
class BillLineItem(BaseModel):
    ...
    date_of_service: Optional[date] = Field(
        None, description="Date service was provided"
    )
...
class InsuranceInfo(BaseModel):
    ...
    covered_amount: Optional[float] = Field(
        None, ge=0, description="Amount covered by insurance"
    )
...
class MedicalBillSchema(BaseModel):
    ...
    hospital_registration: Optional[str] = Field(
        None, description="Hospital registration number"
    )
    ...
    line_items: List[BillLineItem] = Field(
        default_factory=list, description="Itemized charges"
    )

    # Totals
    subtotal: Optional[float] = Field(
        None, ge=0, description="Subtotal before tax/discount"
    )
    ...
    payment_mode: Optional[str] = Field(
        None, description="Payment mode (cash, card, etc.)"
    )
    payment_reference: Optional[str] = Field(
        None, description="Payment reference number"
    )
