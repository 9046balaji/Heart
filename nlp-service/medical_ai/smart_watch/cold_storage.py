...
class VitalsColdStorage:
    ...
    def __init__(
        ...
        # Database setup
        self.database_url = database_url or "postgresql://user:password@localhost/heartguard"
        ...
        try:
            ...
            for reading in readings:
                try:
                    ...
                    if heart_rate is not None:
                        db_readings.append(VitalReading(
                            device_id=device_id,
                            metric_type="hr",
                    ...
                    logger.warning(f"Failed to process reading for device {device_id}: {e}")
                    continue
            ...
            if db_readings:
                ...
                try:
                    ...
                    logger.debug(f"Archived {len(db_readings)} readings for device {device_id}")
                except Exception as e:
                    session.rollback()
                    raise e
                finally:
            ...
            for hour_str, data in hourly_data.items():
                ...
                if data["hr_values"]:
                    db_summaries.append(VitalHourlySummary(
                        device_id=device_id,
                        metric_type="hr",
                ...
                if data["bp_sys_values"]:
                    db_summaries.append(VitalHourlySummary(
                        ...
                        metric_type="bp_sys",
                        hour_start=timestamp,
                        avg_value=sum(data["bp_sys_values"]) / len(data["bp_sys_values"]),
                ...
                if data["bp_dia_values"]:
                    db_summaries.append(VitalHourlySummary(
                        ...
                        metric_type="bp_dia",
                        hour_start=timestamp,
                        avg_value=sum(data["bp_dia_values"]) / len(data["bp_dia_values"]),
                ...
                if data["spo2_values"]:
                    db_summaries.append(VitalHourlySummary(
                        ...
                        metric_type="spo2",
                        hour_start=timestamp,
                        avg_value=sum(data["spo2_values"]) / len(data["spo2_values"]),
                ...
                if data["temp_values"]:
                    db_summaries.append(VitalHourlySummary(
                        ...
                        metric_type="temp",
                        hour_start=timestamp,
                        avg_value=sum(data["temp_values"]) / len(data["temp_values"]),
            ...
            if db_summaries:
                ...
                try:
                    ...
                    logger.debug(f"Created {len(db_summaries)} summaries for device {device_id}")
                except Exception as e:
                    session.rollback()
                    raise e
                finally:
    ...
    async def get_hourly_summary(
        ...
        session = self.SessionLocal()
        try:
            summaries = session.query(VitalHourlySummary).filter(
            ...
            ).order_by(VitalHourlySummary.hour_start).all()
    ...
    async def get_raw_readings(
        ...
        session = self.SessionLocal()
        try:
            readings = session.query(VitalReading).filter(
            ...
            ).order_by(VitalReading.timestamp).limit(limit).all()
...
async def run_hourly_archive(cold_storage: VitalsColdStorage, interval_minutes: int = 60):
    """
