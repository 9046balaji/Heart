...
class MultimodalMedicalProcessor:
    ...
    def __init__(self, medgemma_service=None, vision_model: Optional[str] = None):
        """
    ...
    async def process_document(
        ...
        confidence = self._calculate_confidence(
            ocr_text, visual_analysis, image_quality
        )
...
"""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    ...
                    raise RuntimeError(
                        f"Vision API call failed: {response.status} - {error_text}"
                    )
    ...
    def _assess_image_quality(
        self, image_path: str, visual_analysis: Dict[str, Any]
    ) -> str:
    ...
    def _calculate_confidence(
        self, ocr_text: str, visual_analysis: Dict[str, Any], image_quality: str
    ) -> float:
    ...
    def _generate_recommendations(
        self, image_quality: str, confidence: float
    ) -> List[str]:
        ...
        if image_quality == "poor":
            recommendations.append(
                "Consider rescanning the document with better lighting"
            )
...
async def process_medical_image(
    image_path: str, ocr_text: str, document_type: str = "unknown"
) -> MultimodalAnalysisResult:
    """
