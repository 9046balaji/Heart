...
class ExtractedDrug(BaseModel):
    ...
    dosage_value: Optional[float] = Field(None, ge=0, le=10000, description="Dosage amount")
    ...
    def validate_drug_name(cls, v):
        ...
        hallucination_patterns = ['unknown', 'n/a', 'not specified', 'drug x', 'medication']
...
class MedGemmaExtractionResult(BaseModel):
    ...
    medications: List[ExtractedDrug] = Field(default=[], description="Extracted medications")
    lab_values: List[ExtractedLabValue] = Field(default=[], description="Extracted lab values")
    diagnoses: List[str] = Field(default=[], description="Extracted diagnoses")
    raw_confidence: float = Field(0.0, ge=0.0, le=1.0, description="Overall confidence score")