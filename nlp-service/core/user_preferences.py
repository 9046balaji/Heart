...
class UserPreferencesManager:
    ...
    def get_all_preferences(
        ...
        with self._get_session() as db:
            ...
            return {
                p.preference_key: self._deserialize(
                    p.preference_value, p.data_type
                )
                for p in prefs
            }
    ...
    def clear_all_preferences(
        ...
        """
        with self._get_session() as db:
            deleted = db.query(UserPreference).filter_by(
                user_id=user_id
            ).delete()
    ...
    def export_user_data(
        ...
        """
        with self._get_session() as db:
            prefs = db.query(UserPreference).filter_by(
                user_id=user_id
            ).all()
    ...
    def delete_user_data(
        ...
        with self._get_session() as db:
            # Delete preferences
            pref_count = db.query(UserPreference).filter_by(
                user_id=user_id
            ).delete()
    ...
    def get_audit_log(
        ...
        """
        with self._get_session() as db:
            query = db.query(PreferenceAuditLog).filter_by(
                user_id=user_id
            )
            ...
            logs = query.order_by(
                PreferenceAuditLog.timestamp.desc()
            ).limit(limit).all()
