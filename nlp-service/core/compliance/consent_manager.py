...
class ConsentManager:
    ...
    def withdraw_consent(
        self, user_id: str, consent_type: ConsentType, ip_address: Optional[str] = None
        ...
        if self.audit:
            self.audit.log_consent_withdrawn(
                user_id=user_id, consent_type=consent_type.value, ip_address=ip_address
            )
    ...
    def get_all_consents(
        self, user_id: str, include_withdrawn: bool = False
    ) -> List[ConsentRecord]:
        """
    ...
    def get_users_with_consent(self, consent_type: ConsentType) -> List[str]:
        """
    ...
    def bulk_grant_default_consents(
        ...
        for consent_type in consent_types:
            record = self.grant_consent(
                user_id=user_id, consent_type=consent_type, ip_address=ip_address
            )
    ...
    def check_required_consents(
        self, user_id: str, required: List[ConsentType]
    ) -> Dict[str, bool]:
        """
    ...
    def export_consent_report(self, user_id: str) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "active_consents": len(
                [c for c in consents if c.status == ConsentStatus.GRANTED]
            ),
            "withdrawn_consents": len(
                [c for c in consents if c.status == ConsentStatus.WITHDRAWN]
            ),
            "consents": [
                {
...
REQUIRED_CONSENTS = {
    "weekly_summary": [ConsentType.HEALTH_DATA_PROCESSING, ConsentType.PRIVACY_POLICY],
    "document_scanning": [
