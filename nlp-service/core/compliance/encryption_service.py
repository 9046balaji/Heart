...
class PHIEncryptionService:
    ...
    def __init__(
        ...
        if mock_mode:
            logger.warning(
                "PHI Encryption running in MOCK MODE - data is NOT encrypted"
            )
            self.fernet = None
            return
    ...
    def _encrypt_list(self, items: List[Any], fields: List[str]) -> List[Any]:
    ...
    def mask_phi(self, data: Dict[str, Any], show_last: int = 4) -> Dict[str, Any]:
        """
        ...
        for key, value in data.items():
            ...
            if should_mask and isinstance(value, str) and len(value) > show_last:
                ...
                plain_value = (
                    self.decrypt_field(value) if self.is_encrypted(value) else value
                )
                # Mask
                masked_data[key] = (
                    "*" * (len(plain_value) - show_last) + plain_value[-show_last:]
                )
            elif isinstance(value, dict):
    ...
    def rotate_key(self, data: Dict[str, Any], new_master_key: str) -> Dict[str, Any]:
        """
