...
class DataRetentionService:
    ...
    def register_policy(self, policy: RetentionPolicy) -> None:
        ...
        logger.info(f"Registered retention policy: {policy.name} for {policy.data_type}")
    ...
    def process_erasure_request(
        ...
        for record in user_records:
            if record.legal_hold:
                ...
                logger.warning(
                    f"Cannot erase {record.resource_id}: under legal hold"
                )
                continue
