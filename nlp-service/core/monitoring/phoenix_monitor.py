...
try:
    ...
    logger.warning(
        "Arize Phoenix not available - install with: pip install arize-phoenix"
    )
...
class PhoenixMonitor:
    ...
    def log_retrieval(
        self, query: str, documents: list, scores: list, latency_ms: float
    ):
        ...
        try:
            self.session.log_span(
                ...
                inputs={"query": query},
                outputs={"documents": documents, "scores": scores},
                metadata={"latency_ms": latency_ms, "num_documents": len(documents)},
            )
        except Exception as e:
            logger.warning(f"Failed to log retrieval: {e}")

    def log_evaluation(self, metrics: Dict[str, float], dataset_name: str):
