...
def with_timeout(
    seconds: float = 10.0, fallback: Optional[Any] = None, log_level: str = "warning"
) -> Callable:
    """
...
def with_timeout_context(
    seconds: float = 10.0, error_callback: Optional[Callable] = None
):
    """
...
class TimeoutMiddleware:
    ...
    def __init__(
        self, app, timeout_seconds: float = 30.0, exclude_paths: Optional[list] = None
    ):
        """
...
async def parallel_with_timeout(
    *coroutines, timeout_seconds: float = 10.0, return_exceptions: bool = False
) -> list:
    """
...
class TimeoutStats:
    ...
    def record_timeout(self, endpoint: str):
        ...
        self.timeout_by_endpoint[endpoint] = (
            self.timeout_by_endpoint.get(endpoint, 0) + 1
        )
