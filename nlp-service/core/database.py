    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "utilization": {
                    "current": (
                        recent_snapshots[-1].utilization_pct if recent_snapshots else 0
                    ),
                ...
                "connection_wait_ms": {
                    "avg": (
                        round(sum(recent_waits) / len(recent_waits), 2)
                        if recent_waits
                        else 0
                    ),
                ...
                "query_duration_ms": {
                    "avg": (
                        round(sum(recent_queries) / len(recent_queries), 2)
                        if recent_queries
                        else 0
                    ),
                    "p95": (
                        self._percentile(recent_queries, 95) if recent_queries else 0
                    ),
                },
                "timeout_rate": round(
                    self.total_timeouts / max(self.total_connections, 1) * 100, 2
                ),
            }
...
class AdaptivePoolManager:
    ...
    def evaluate_and_resize(self, current_utilization: float) -> Optional[int]:
        ...
        with self._lock:
            ...
            if new_size and new_size != old_size:
                ...
                logger.info(
                    f"Pool resized: {old_size} -> {new_size} " f"(reason: {reason})"
                )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_resize": (
                self._last_resize_time.isoformat() if self._last_resize_time else None
            ),
...
class DatabaseConnection:
    ...
    def __init__(
        ...
        pool_class = DatabaseConfig.POOL_STRATEGIES.get(self.environment, QueuePool)
    ...
    def get_adaptive_sizing_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "resize_history": self.adaptive_manager.get_resize_history()[
                -10:
            ],  # Last 10
        }
...
db = DatabaseConnection(
    database_url=DATABASE_URL, environment=DatabaseConfig.ENVIRONMENT
)
