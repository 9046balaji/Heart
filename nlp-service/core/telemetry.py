...
class MetricsRegistry:
    ...
    @classmethod
    def gauge(
        cls, name: str, callback: Callable, description: str = "", unit: str = ""
    ) -> Any:
...
def get_request_counter():
    ...
    return MetricsRegistry.counter(
        "nlp_requests_total", "Total number of NLP requests", "requests"
    )
...
def get_response_histogram():
    ...
    return MetricsRegistry.histogram(
        "nlp_response_duration_seconds", "Response time in seconds", "s"
    )
...
def get_error_counter():
    """Counter for errors."""
    return MetricsRegistry.counter(
        "nlp_errors_total", "Total number of errors", "errors"
    )
...
@contextmanager
def trace_llm_call(model: str, prompt_tokens: int = 0, provider: str = "gemini"):
...
@contextmanager
def trace_rag_retrieval(query: str, collection: str = "default", top_k: int = 5):
...
@contextmanager
def trace_agent_execution(agent_name: str, action: str):
