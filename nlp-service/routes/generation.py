...
class RecipeAnalysisResponse(BaseModel):
    ...
    allergen_warnings: List[str] = Field(
        default_factory=list, description="Allergen warnings"
    )
    provider: str = Field(..., description="LLM provider used")
...
class MealPlanRequest(BaseModel):
    ...
    calorie_target: int = Field(
        default=2000, ge=1000, le=5000, description="Daily calorie target"
    )
...
class ChatRequest(BaseModel):
    ...
    content_type: str = Field(
        default="general", description="Content type for disclaimers"
    )
...
class WorkoutAnalysisRequest(BaseModel):
    ...
    heart_rate_data: List[float] = Field(
        default_factory=list, description="Heart rate samples"
    )
    user_goals: List[str] = Field(
        default_factory=list, description="User fitness goals"
    )
    user_id: str = Field(default="anonymous", description="User identifier")
...
class HealthAssessmentRequest(BaseModel):
    ...
    health_history: List[str] = Field(
        default_factory=list, description="Health history"
    )
    lifestyle: Dict[str, Any] = Field(
        default_factory=dict, description="Lifestyle factors"
    )
    user_id: str = Field(default="anonymous", description="User identifier")
...
class MedicationInsightsRequest(BaseModel):
    ...
    supplements: List[str] = Field(
        default_factory=list, description="List of supplements"
    )
    recent_vitals: Dict[str, Any] = Field(
        default_factory=dict, description="Recent vitals"
    )
    user_id: str = Field(default="anonymous", description="User identifier")
...
def build_recipe_prompt(
    request: RecipeAnalysisRequest, allergen_warnings: List[str]
) -> str:
...
def build_workout_prompt(request: WorkoutAnalysisRequest) -> str:
    ...
    avg_hr = (
        sum(request.heart_rate_data) / len(request.heart_rate_data)
        if request.heart_rate_data
        else "N/A"
    )
...
def build_medication_prompt(request: MedicationInsightsRequest) -> str:
    ...
    med_list = [
        f"{m.get('name')} ({m.get('dosage', 'N/A')})" for m in request.medications
    ]
...
async def analyze_recipe(request: RecipeAnalysisRequest) -> RecipeAnalysisResponse:
    ...
    try:
        ...
        analysis = await gateway.generate(
            prompt=prompt, content_type="nutrition", user_id=request.user_id
        )
...
async def analyze_workout(request: WorkoutAnalysisRequest) -> WorkoutAnalysisResponse:
    ...
    try:
        ...
        analysis = await gateway.generate(
            prompt=prompt, content_type="fitness", user_id=request.user_id
        )
        status = gateway.get_status()
...
async def get_generation_status() -> Dict[str, Any]:
    ...
    gateway = get_llm_gateway()
    return {"service": "generation", "llm_gateway": gateway.get_status()}
    ...
    try:
        ...
        async def generate_sse():
            async for chunk in gateway.generate_stream(
                prompt=request.message, content_type=request.content_type
            ):
