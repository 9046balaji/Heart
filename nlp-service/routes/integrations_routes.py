...
class PredictionFromDocumentRequest(BaseModel):
    ...
    patient_profile: dict = Field(
        ...
        description="Patient demographics (age, sex, etc.)"
    )
...
async def get_patient_timeline(
    ...
    importance: Optional[str] = Query(default=None, description="Comma-separated importance levels"),
    ...
    """
    try:
        from integrations.timeline_service import get_timeline_service, TimelineEventType, EventImportance
...
async def get_timeline_summary(
    ...
    try:
        ...
        return TimelineSummaryResponse(
            ...
            date_range={
                "start": summary.date_range["start"].isoformat() if summary.date_range["start"] else None,
                "end": summary.date_range["end"].isoformat() if summary.date_range["end"] else None
            },
...
async def get_lab_trends_for_doctor(
    patient_id: str,
    physician_id: str = Query(...),
    test_names: Optional[str] = Query(default=None, description="Comma-separated test names"),
