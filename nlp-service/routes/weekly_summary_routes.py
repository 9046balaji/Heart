...
class DeliveryPreference(BaseModel):
    ...
    channel: str = Field(..., description="Delivery channel: whatsapp, email, sms, push")
    enabled: bool = True
...
class SummaryPreferencesRequest(BaseModel):
    ...
    preferred_day: int = Field(0, ge=0, le=6, description="Day of week (0=Monday, 6=Sunday)")
...
async def get_preferences(
    ...
    return SummaryPreferencesResponse(
        ...
        enabled=True,
        delivery_channels=[
            DeliveryPreference(channel="whatsapp", enabled=True, destination="+1234567890"),
            DeliveryPreference(channel="email", enabled=True, destination="user@example.com")
        ],
...
async def update_preferences(
    request: SummaryPreferencesRequest,
    user_id: str = Query(..., description="User ID")
):
...
async def unsubscribe(
    user_id: str = Query(..., description="User ID"),
    channel: Optional[str] = Query(None, description="Specific channel to unsubscribe from")
):
...
async def export_consent_report(
    user_id: str = Query(..., description="User ID")
):
...
async def get_required_consents(
    feature: str = Query(..., description="Feature to check (weekly_summary, document_scanning, ai_analysis)")
):
