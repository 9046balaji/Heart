...
class WrapContentRequest(BaseModel):
    ...
    content: str
    content_type: str
    format: str = Field(default="text", description="Output format: text, markdown, html, whatsapp")
...
async def get_pending_verifications(
    user_id: Optional[str] = None,
    priority: bool = Query(default=True, description="Sort by priority (most flags first)")
):
    """
...
async def mask_phi_fields(
    ...
    show_last: int = Query(default=4, ge=0, le=10)
):
    """
