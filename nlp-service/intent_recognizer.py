...
from typing import Dict, List, Tuple, Awaitable, Optional
...
class IntentRecognizer:
    ...
    def __init__(self):
        ...
        self.intent_keywords: Dict[IntentEnum, List[str]] = {
            IntentEnum.GREETING: [
                ...
                "good evening", "howdy", "greetings", "welcome"
            ],
            IntentEnum.RISK_ASSESSMENT: [
                "risk", "heart disease", "assessment", "chance", "probability",
                "what's my risk", "am i at risk", "risk of heart attack",
                "likelihood", "calculate risk", "evaluate risk"
            ],
            IntentEnum.NUTRITION_ADVICE: [
                "eat", "food", "meal", "nutrition", "diet", "calories",
                "healthy eating", "recipes", "meal plan", "what should i eat",
                "food recommendations", "dietary", "nutrition plan"
            ],
            IntentEnum.EXERCISE_COACHING: [
                "exercise", "workout", "fitness", "training", "cardio",
                "physical activity", "workout plan", "exercise routine",
                "gym", "running", "cycling", "sports", "activity"
            ],
            IntentEnum.MEDICATION_REMINDER: [
                "medication", "pill", "medicine", "dose", "prescription",
                "take medication", "medicine reminder", "dosage",
                "refill", "pharmacy", "drug"
            ],
            IntentEnum.SYMPTOM_CHECK: [
                "pain", "symptom", "feel", "hurt", "discomfort",
                "ache", "feeling", "experience", "having", "chest"
            ],
            IntentEnum.HEALTH_GOAL: [
                "goal", "target", "achieve", "improve", "want to",
                "weight loss", "fitness goal", "health goal",
                "progress", "track", "monitor"
            ],
            IntentEnum.HEALTH_EDUCATION: [
                "learn", "teach", "education", "information", "know",
                "understand", "explain", "tell me about", "what is",
                "how does", "educational", "facts"
            ],
            IntentEnum.APPOINTMENT_BOOKING: [
                "appointment", "doctor", "booking", "schedule", "visit",
                "meeting", "consultation", "healthcare provider",
        ...
        self.use_transformer = os.getenv("USE_TRANSFORMER_MODELS", "false").lower() == "true"
        self.transformer_recognizer = None

        if self.use_transformer and TRANSFORMER_AVAILABLE:
            try:
                transformer_model = os.getenv("TRANSFORMER_MODEL_NAME", "bert-base-uncased")
                self.transformer_recognizer = TransformerIntentRecognizer(transformer_model)
                print(f"Transformer intent recognizer initialized with model: {transformer_model}")
    ...
    def _load_vectors_from_cache(self, cache_file: Path):
        ...
        try:
            with open(cache_file, 'rb') as f:
                ...
                print(f"Loaded intent vectors from cache ({cache_file.stat().st_size / 1024:.1f}KB)")
        except Exception as e:
    ...
    def _compute_and_cache_vectors(self, cache_file: Path):
        ...
        try:
            ...
            print(f"Cached intent vectors to {cache_file} ({cache_file.stat().st_size / 1024:.1f}KB)")
        except Exception as e:
    ...
    def recognize_intent_trie(self, text: str) -> IntentResult:
        ...
        cache_key = f"intent_fast:{hashlib.md5(text.lower().encode()).hexdigest()}"
    ...
    def recognize_intent(self, text: str) -> IntentResult:
        ...
        cache_key = f"intent:{hashlib.md5(text.lower().encode()).hexdigest()}"
        cached_result = cache_manager.get(cache_key)
        if cached_result is not None:
            return IntentResult(**cached_result)
        ...
        if self.use_transformer and self.transformer_recognizer:
            try:
                ...
                print(f"Transformer model failed, falling back to keyword matching: {e}")
    ...
    def _calculate_intent_score(self, text: str, keywords: List[str]) -> Tuple[float, List[str]]:
