...
- Be clear about what is general information vs personalized advice"""
    ...
    async def _retrieve_medical(self, query: str) -> List[Dict]:
        ...
        try:
            ...
            results = self.vector_store.search_medical_knowledge(
                query, top_k=self.medical_top_k * 3  # Retrieve 3x for reranking
            )

            # Filter by relevance score
            filtered_results = [
                r for r in results if r.get("score", 0) >= self.min_relevance_score
            ]

            # Rerank if reranker is available
            if (
                self.reranker.is_available()
                and len(filtered_results) > self.medical_top_k
            ):
                filtered_results = self.reranker.rerank(
                    query, filtered_results, self.medical_top_k
                )
    ...
    async def _retrieve_memories(self, query: str, user_id: str) -> List[Dict]:
        ...
        try:
            results = self.vector_store.search_user_memories(
                user_id, query, top_k=self.memory_top_k
            )
    ...
    async def _retrieve_drugs(self, query: str) -> List[Dict]:
        ...
        try:
            results = self.vector_store.search_drug_info(query, top_k=self.drug_top_k)
            return [r for r in results if r.get("score", 0) >= self.min_relevance_score]
        except Exception as e:
...
if __name__ == "__main__":
    ...
    async def test_rag():
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print("\n💬 Testing full query...")
            response = await rag.query(
                "What are the symptoms of heart failure?", user_id="test_user"
            )
