...
class ContextualMedicalChunker:
    ...
    def _parse_sections(self, text: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            stripped = line.strip()
            is_heading = (
                any(marker in stripped for marker in self.SECTION_MARKERS)
                or (stripped.endswith(":") and len(stripped) < 50)
    ...
    def _extract_entities(self, text: str) -> List[Dict]:
        ...
        for match in self.CONDITION_PATTERN.finditer(text):
            entities.append(
                {"type": "condition", "name": match.group(0), "position": match.start()}
            )

        return entities

    def _infer_chunk_type(self, section: str, entities: List[Dict]) -> str:
    ...
    def _split_sentences_with_offsets(self, text: str) -> List[Dict]:
        ...
        for match in matches:
            ...
            if raw_sent.strip():
                sentences.append({"text": raw_sent, "start": start, "end": end})
        ...
        if start < len(text):
            raw_sent = text[start:]
            if raw_sent.strip():
                sentences.append({"text": raw_sent, "start": start, "end": len(text)})
