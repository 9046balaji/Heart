...
class LlamaIndexRAG:
    ...
    def __init__(self, documents_path: Optional[str] = None):
        ...
        # Configure settings
        Settings.llm = Ollama(
            model=os.getenv("OLLAMA_MODEL", "gemma3:1b"), request_timeout=120.0
        )
        ...
        if documents_path and os.path.exists(documents_path):
            try:
                ...
                logger.info(
                    f"✅ Loaded {len(documents)} documents from {documents_path}"
                )
            ...
            dummy_doc = Document(
                text="Cardiovascular health information will be indexed here."
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "llm_model": (
                Settings.llm.metadata.model_name if Settings.llm else "unknown"
            ),
            "embed_model": (
                Settings.embed_model.model_name if Settings.embed_model else "unknown"
            ),
            "chunk_size": (
                Settings.node_parser.chunk_size if Settings.node_parser else 512
            ),
        }
