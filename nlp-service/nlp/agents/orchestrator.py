...
6. Always recommends professional consultation for concerning findings"""
    ...
    async def analyze(
        ...
        if health_data and "blood_pressure" in health_data:
            ...
            if isinstance(bp, dict):
                ...
                if systolic >= 180 or diastolic >= 120:
                    warnings.append(
                        "⚠️ Blood pressure is critically high. Seek immediate medical attention."
                    )
                elif systolic >= 140 or diastolic >= 90:
                    warnings.append(
                        "Blood pressure is elevated. Consider consulting your doctor."
                    )
                elif systolic < 90 or diastolic < 60:
                    insights.append(
                        "Blood pressure is on the lower side. Monitor for symptoms."
                    )
                else:
                    insights.append(
                        f"Blood pressure ({systolic}/{diastolic}) is within normal range."
                    )
        ...
        if health_data and "heart_rate" in health_data:
            ...
            if hr > 100:
                insights.append(
                    f"Heart rate ({hr} bpm) is elevated. Consider rest and relaxation."
                )
            elif hr < 60:
                insights.append(
                    f"Heart rate ({hr} bpm) is low. Normal for athletes, check if feeling dizzy."
                )
            else:
...
6. Confirms emergency situations are handled correctly""",
    ...
    async def validate(
        ...
        return {
            ...
            "recommendation": (
                "Response validated" if validated else "Review flagged issues"
            ),
        }
                ...
                if agent:
                    ...
                    if task.agent_role == AgentRole.HEALTH_ANALYST:
                        result = await agent.analyze(
                            task.query, health_data, full_context
                        )
                        ...
                        result = await agent.advise(
                            task.query, medications, full_context
                        )
                    elif task.agent_role == AgentRole.SYMPTOM_CHECKER:
                        result = await agent.check_symptoms(
                            task.query, context=full_context
                        )
                        ...
                        result = await agent.synthesize(
                            agent_responses, validation, query
                        )
                    else:
...
if __name__ == "__main__":
    ...
    async def test_orchestrator():
        ...
        print("\n🧪 Test 2: Medication query")
        result = await orchestrator.process(
            "What should I know about taking lisinopril?", medications=["lisinopril"]
        )
        print(f"  Response: {result.response[:200]}...")
        print(f"  Confidence: {result.confidence:.2f}")
        ...
        print("\n🧪 Test 3: Symptom query")
        result = await orchestrator.process("I've been feeling dizzy and tired lately")
        print(f"  Response: {result.response[:200]}...")
        print(f"  Warnings: {result.warnings}")

        # Test 4: Lifestyle query
        print("\n🧪 Test 4: Lifestyle query")
        result = await orchestrator.process("What exercises are good for heart health?")
        print(f"  Response: {result.response[:200]}...")
