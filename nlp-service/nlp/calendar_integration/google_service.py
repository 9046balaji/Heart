...
class GoogleCalendarService:
    ...
    def __init__(self, mock_mode: bool = None):
        ...
        if mock_mode is None:
            mock_mode = (
                os.getenv("GOOGLE_CALENDAR_MOCK_MODE", "false").lower() == "true"
            )
    ...
    def authorize_user(self, user_email: str) -> Dict[str, Any]:
        ...
        if self.mock_mode:
            return {
                "status": "success",
                "message": "Mock mode - no authorization needed",
            }
        ...
        try:
            ...
            return {
                "status": "success",
                "message": f"Authorization successful for {user_email}",
            }
        except Exception as e:
    ...
    def create_event(self, user_email: str, event: CalendarEvent) -> Optional[str]:
        """
        ...
        try:
            ...
            if hasattr(event, "attendee_emails") and event.attendee_emails:
                body["attendees"] = [
                    {"email": email} for email in event.attendee_emails
                ]
            ...
            if hasattr(event, "reminder_minutes") and event.reminder_minutes:
                body["reminders"] = {
                    ...
                    "overrides": [
                        ...
                        for m in event.reminder_minutes
                    ],
                }
    ...
    def update_event(
        self, user_email: str, event_id: str, event: CalendarEvent
    ) -> bool:
        """
    ...
    def list_events(
        ...
        try:
            ...
            if self.mock_mode:
                logger.info(
                    f"Listed mock events for {user_email} from {start_time} to {end_time}"
                )
                return []
            ...
            logger.info(
                f"Listed {len(events)} events for {user_email} from {start_time} to {end_time}"
            )
            return events
            ...
            for appointment in appointments:
                try:
                    ...
                    if event_id:
                        successful += 1
                        logger.info(
                            f"Synced appointment {appointment.appointment_id} to Google Calendar"
                        )
                    else:
                        failed += 1
                    ...
                    failed += 1
                    sync_record.sync_errors.append(
                        {"appointment_id": appointment.appointment_id, "error": str(e)}
                    )
