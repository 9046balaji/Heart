...
from calendar_integration.models import (
    Reminder, ReminderStatus, NotificationType
)
...
class ReminderScheduler:
    ...
    def _on_job_missed(self, event) -> None:
        ...
        with self.lock:
            if job_id in self.scheduled_reminders:
                self.scheduled_reminders[job_id].status = ReminderStatus.FAILED
                self.scheduled_reminders[job_id].last_error = "Job missed - scheduler was stopped"
    ...
    def schedule_reminder(
        ...
        try:
            with self.lock:
                ...
                if delay_seconds < 0:
                    logger.warning(f"Scheduled time is in the past: {reminder.scheduled_time}")
    ...
    def schedule_reminders(
        ...
        try:
            with self.lock:
                ...
                logger.info(f"Scheduled {len(scheduled_ids)} reminders for appointment {appointment_id}")
                ...
                if reminder_id in self.scheduled_reminders:
                    ...
                    if not self.mock_mode and self.scheduler and self._started:
                        try:
                            ...
                            logger.debug(f"Job {reminder_id} already removed or not found: {e}")
        ...
        try:
            with self.lock:
                reminder_ids_to_cancel = [
                    rid for rid, reminder in self.scheduled_reminders.items()
    ...
    def get_appointment_reminders(self, appointment_id: str) -> List[Reminder]:
        ...
        with self.lock:
            return [
                reminder for reminder in self.scheduled_reminders.values()
                if reminder.appointment_id == appointment_id
            ]
    ...
    def clear_completed_reminders(self) -> int:
        ...
        try:
            with self.lock:
                ids_to_remove = [
                    rid for rid, reminder in self.scheduled_reminders.items()
                    if reminder.status in [ReminderStatus.SENT, ReminderStatus.FAILED, ReminderStatus.CANCELLED]
                ]
