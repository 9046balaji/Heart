...
from calendar_integration.models import (
    CalendarEvent, SyncAppointmentsRequest, ScheduleRemindersRequest,
    CalendarProvider, NotificationType, Reminder
)
...
from calendar_integration.notifications import (
    get_email_service, get_sms_service, get_push_service
)
        ...
        try:
            ...
            if request.appointment_ids:
                ...
                for appt_id in request.appointment_ids:
                    ...
                    if appt and appt.status in [AppointmentStatus.CONFIRMED, AppointmentStatus.PENDING]:
                ...
                appointments = [a for a in all_appts if a.status == AppointmentStatus.CONFIRMED]
            
            # Convert to calendar events
            calendar_events = [self._appointment_to_event(appt, request.provider) for appt in appointments]
            ...
            elif request.provider in [CalendarProvider.OUTLOOK, CalendarProvider.MICROSOFT]:
    ...
    def _appointment_to_event(
        ...
        return CalendarEvent(
            ...
            end_time=appointment.scheduled_datetime.replace(
                minute=appointment.scheduled_datetime.minute + appointment.estimated_duration_minutes
            ),
...
class ReminderScheduleAgent(BaseAgent):
    ...
    def execute(
        ...
        """
        try:
            self.log_action("schedule_reminders", f"appointment={request.appointment_id}")
            
            # Get appointment details
            appointment = self.appointment_service.get_appointment(request.appointment_id, "SYSTEM")
            if not appointment:
    ...
    def cancel_reminders(self, appointment_id: str) -> Dict:
        ...
        try:
            ...
            cancelled_count = self.scheduler.cancel_appointment_reminders(appointment_id)
