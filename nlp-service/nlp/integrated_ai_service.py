...
class IntegratedHealthAIService:
    ...
    async def process_query(
        ...
        try:
            ...
            metrics.ai_call_ms = (datetime.utcnow() - ai_start).total_seconds() * 1000
            ...
            metrics.total_ms = (datetime.utcnow() - start_time).total_seconds() * 1000
    ...
    async def _store_user_message(
        self, session_id: str, user_id: str, query: str, start_time: datetime
    ) -> None:
    ...
    async def _retrieve_context(
        self, user_id: str, session_id: str, query: str, is_emergency: bool
    ...
    async def _get_communication_style(
        ...
        try:
            style_str = self.preferences.get_preference(
                user_id=user_id, key=PreferenceKeys.COMMUNICATION_STYLE, default=None
            )
            if style_str:
    ...
    async def _call_ai(
        self, system_message: str, user_message: str, provider: str
    ) -> str:
    ...
    async def _call_gemini(self, system_message: str, user_message: str) -> str:
    ...
    async def _call_ollama(self, system_message: str, user_message: str) -> str:
        ...
        try:
            ...
            response = await self.ollama.chat(
                model="llama2", messages=messages  # Or configured model
            )
    ...
    def _build_audit_trail(
        ...
        phi_accessed = any(
            ctx.context_type in phi_context_types for ctx in retrieved_contexts
        )
        ...
        return {
            ...
            "is_emergency": is_emergency,
            "context_sources": list(set(ctx.source for ctx in retrieved_contexts)),
        }
    ...
    async def quick_query(self, user_id: str, query: str) -> str:
        """
    ...
    async def continue_conversation(self, session_id: str, query: str) -> AIResponse:
        """
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        health["components"]["gemini"] = (
            "configured" if self.gemini else "not configured"
        )
        health["components"]["ollama"] = (
            "configured" if self.ollama else "not configured"
        )
...
def init_integrated_ai_service(
    gemini_client=None, ollama_client=None, database_url: Optional[str] = None, **kwargs
