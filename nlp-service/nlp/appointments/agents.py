...
class AppointmentValidatorAgent(BaseAgent):
    ...
    def _validate_datetime(self, datetime_str: str) -> Dict:
        ...
        result = {"is_valid": True, "flags": [], "recommendations": []}
        ...
        try:
            ...
            if not datetime_str:
                ...
                result["recommendations"].append(
                    "Request patient to specify preferred date/time"
                )
...
class AppointmentBookingAgent(BaseAgent):
    ...
    def execute(
        ...
        try:
            self.log_action(
                "book_appointment", f"patient={parsed_data.get('patient_id')}"
            )
            ...
            request = AppointmentCreateRequest(
                ...
                reason_for_visit=parsed_data.get(
            ...
            appointment_id = self.appointment_service.create_appointment(
                request, user_id=parsed_data["patient_id"]
            )
            ...
            if appointment_id:
                ...
                self.log_action(
                    "booking_confirmed", f"id={appointment_id}, provider={provider_id}"
                )
            ...
            return {"booked": False, "reason": f"Booking error: {str(e)}"}
