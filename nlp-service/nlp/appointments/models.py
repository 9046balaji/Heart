...
class TimeSlot(BaseModel):
    ...
    status: AvailabilityStatus = Field(AvailabilityStatus.AVAILABLE, description="Slot availability")
    appointment_duration_minutes: int = Field(30, ge=15, le=480, description="Duration in minutes")
...
class ProviderSchedule(BaseModel):
    ...
    available_slots: List[TimeSlot] = Field(default_factory=list, description="Available time slots")
    blocked_dates: List[datetime] = Field(default_factory=list, description="Dates provider is unavailable")
    working_hours_start: time = Field(default=time(9, 0), description="Daily work start time (HH:MM)")
    working_hours_end: time = Field(default=time(17, 0), description="Daily work end time (HH:MM)")
    days_off: List[str] = Field(default_factory=list, description="Days off (Mon, Tue, etc.)")
...
class AppointmentReminder(BaseModel):
    ...
    time_before_minutes: int = Field(15, ge=1, le=10080, description="Minutes before appointment")
...
class Appointment(BaseModel):
    ...
    status: AppointmentStatus = Field(AppointmentStatus.PENDING, description="Current status")
    scheduled_datetime: datetime = Field(..., description="Scheduled appointment time")
    estimated_duration_minutes: int = Field(30, ge=15, le=480, description="Expected duration")
    
    # Location and access
    location: Optional[str] = Field(None, description="Physical location or telehealth info")
    ...
    # Medical context
    reason_for_visit: str = Field(..., description="Chief complaint or reason for visit")
    clinical_notes: Optional[str] = Field(None, description="Provider's notes")
    relevant_medications: List[str] = Field(default_factory=list, description="Current medications")
    known_allergies: List[str] = Field(default_factory=list, description="Known allergies")
    
    # Reminders
    reminders: List[AppointmentReminder] = Field(default_factory=list, description="Scheduled reminders")
    
    # HIPAA compliance
    data_classification: str = Field("CONFIDENTIAL_MEDICAL", description="Data sensitivity level")
    patient_confirmed: bool = Field(False, description="Patient confirmed attendance")
    confirmation_datetime: Optional[datetime] = Field(None, description="When patient confirmed")
    ...
    cancelled_by: Optional[str] = Field(None, description="Who cancelled (patient_id or provider_id)")
