...
try:
    ...
    if sys.version_info >= (3, 14):
        print(
            "Warning: Python 3.14+ detected. Spacy has compatibility issues. Using regex fallback."
        )
...
class EntityExtractor:
    ...
    def __init__(self):
        ...
        self.weight_pattern = re.compile(
            r"(\d+\.?\d*)\s*(lbs?|pounds?|kg|kilograms?)", re.IGNORECASE
        )
    ...
    def extract_entities(
        ...
        if entity_types is not None:
            ...
            if len(entity_types) > 20:
                raise ProcessingError(
                    ...
                    details={"requested": len(entity_types), "max_allowed": 20},
                )
        ...
        # Create cache key
        cache_key = f"entities:{hashlib.sha256(f'{text}:{sorted(entity_types or [])}'.encode()).hexdigest()}"
        ...
        cache_manager.set(
            cache_key, [entity.model_dump() for entity in entities], ttl=3600
        )
    ...
    def _extract_time_references(self, text: str) -> List[Entity]:
        ...
        duration_pattern = re.compile(
            r"(\d+)\s*(hours?|days?|weeks?|months?|years?)", re.IGNORECASE
        )
    ...
    def get_entity_summary(self, entities: List[Entity]) -> dict:
        ...
        """
        summary = {"total": len(entities), "by_type": {}}
    ...
    def _compile_regex_patterns(self) -> None:
        ...
        try:
            ...
            for symptom in CARDIOVASCULAR_SYMPTOMS:
                pattern = re.compile(
                    r"\b" + re.escape(symptom.lower()) + r"\b", re.IGNORECASE
                )
            ...
            for medication in MEDICATIONS_DATABASE.keys():
                pattern = re.compile(
                    r"\b" + re.escape(medication.lower()) + r"\b", re.IGNORECASE
                )
            ...
            for food in HEART_HEALTHY_FOODS.keys():
                pattern = re.compile(
                    r"\b" + re.escape(food.lower()) + r"\b", re.IGNORECASE
                )
                self.food_patterns.append((pattern, food))

            logger.info(
                f"Compiled regex patterns: {len(self.symptom_patterns)} symptoms, "
