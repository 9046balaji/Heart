...
class VitalsRepository(Protocol):
    ...
    def get_heart_rate_range(self, user_id: str, start: datetime, end: datetime) -> List[int]: ...
    def get_daily_steps_range(self, user_id: str, start: datetime, end: datetime) -> List[int]: ...
    def get_sleep_data_range(self, user_id: str, start: datetime, end: datetime) -> List[Dict]: ...
    def get_blood_pressure_range(self, user_id: str, start: datetime, end: datetime) -> List[Dict]: ...


class NutritionRepository(Protocol):
    """Protocol for nutrition data repository."""
    def get_daily_calories_range(self, user_id: str, start: datetime, end: datetime) -> List[int]: ...
    def get_nutrition_details_range(self, user_id: str, start: datetime, end: datetime) -> List[Dict]: ...


class MedicationRepository(Protocol):
    """Protocol for medication data repository."""
    def get_compliance_range(self, user_id: str, start: datetime, end: datetime) -> Dict[str, Any]: ...


class ExerciseRepository(Protocol):
    """Protocol for exercise data repository."""
    def get_workouts_range(self, user_id: str, start: datetime, end: datetime) -> List[Dict]: ...
...
class WeeklyDataAggregator:
    ...
    def aggregate_weekly_data(
        ...
        logger.info(f"Aggregating weekly data for user {user_id}: {week_start} to {week_end}")
        ...
        completeness = self._calculate_completeness(vitals, nutrition, medications, exercise)
        ...
        try:
            ...
            if hr_data:
                ...
                if mid > 0:
                    ...
                    percent_change = (diff / first_half_avg * 100) if first_half_avg > 0 else 0
            ...
            if bp_data:
                ...
                diastolic = [bp.get('diastolic') for bp in bp_data if bp.get('diastolic')]
                if systolic:
                    vitals.avg_blood_pressure_systolic = round(statistics.mean(systolic), 0)
                if diastolic:
                    vitals.avg_blood_pressure_diastolic = round(statistics.mean(diastolic), 0)
            
            # Check for alerts
            if vitals.max_heart_rate and vitals.max_heart_rate > 120:
                vitals.alerts.append(f"High heart rate detected: {vitals.max_heart_rate} bpm")
            ...
            if vitals.avg_sleep_hours and vitals.avg_sleep_hours < 6:
                vitals.alerts.append(f"Low sleep: Averaging {vitals.avg_sleep_hours:.1f} hours/night")
            
            if vitals.avg_blood_pressure_systolic and vitals.avg_blood_pressure_systolic > 140:
    ...
    def _aggregate_nutrition(
        ...
        try:
            ...
            calorie_data = self.nutrition_repo.get_daily_calories_range(user_id, start, end)
            if calorie_data:
                ...
                if nutrition.avg_daily_calories > 2500:
                    ...
                    nutrition.suggestions.append("You may need more calories for energy")
            
            # Get detailed nutrition
            details = self.nutrition_repo.get_nutrition_details_range(user_id, start, end)
            if details:
                ...
                if sodium:
                    ...
                    if nutrition.avg_sodium_mg > 2300:
                        nutrition.suggestions.append("Sodium intake is above recommended limit")
            ...
            if med_data:
                ...
                if medications.total_medications > 0:
                    medications.compliance_percentage = round(
                        medications.taken_count / medications.total_medications * 100,
                        1
                    )
    ...
    def _aggregate_exercise(
        ...
        try:
            ...
            if workout_data:
                ...
                exercise.total_active_minutes = sum(w.get('duration_minutes', 0) for w in workout_data)
                exercise.calories_burned = sum(w.get('calories', 0) for w in workout_data)
                exercise.workout_types = list(set(w.get('type', 'Unknown') for w in workout_data))
                
                # Count unique workout days
                workout_dates = set(w.get('date', '').split('T')[0] for w in workout_data if w.get('date'))
                ...
                exercise.goal_completion_percentage = min(
                    round(exercise.total_active_minutes / exercise.target_minutes * 100, 1), 
                    100
                )
    ...
    def _calculate_completeness(
        ...
        nutrition_score = min(nutrition.total_days / 7, 1.0) if nutrition.total_days else 0
