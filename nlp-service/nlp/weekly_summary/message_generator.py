...
class WeeklySummaryMessageGenerator:
    ...
    def generate_whatsapp_message(
        ...
        # Nutrition section
        nutrition_section = self._generate_nutrition_section(data, include_detailed_nutrition)
        if nutrition_section:
            sections.append(nutrition_section)
        
        # Medication section
        medication_section = self._generate_medication_section(data, include_medication_details)
...
{greeting}! Here's your health report for {week_str}:"""
    ...
    def _generate_vitals_section(self, data: WeeklyHealthData) -> Optional[str]:
        ...
        if vitals.avg_heart_rate:
            ...
            lines.append(f"{self.EMOJI_HEART} Heart Rate: {vitals.avg_heart_rate:.0f} bpm avg {trend_emoji}")
            if vitals.min_heart_rate and vitals.max_heart_rate:
                lines.append(f"   Range: {vitals.min_heart_rate} - {vitals.max_heart_rate} bpm")
        
        # Steps
        if vitals.avg_steps:
            has_data = True
            trend_emoji = self._get_trend_emoji(vitals.steps_trend)
            lines.append(f"{self.EMOJI_STEPS} Daily Steps: {vitals.avg_steps:,.0f} avg {trend_emoji}")
        ...
        if vitals.avg_sleep_hours:
            ...
            lines.append(f"{self.EMOJI_SLEEP} Sleep: {vitals.avg_sleep_hours:.1f} hrs/night {sleep_quality}")
        ...
        if vitals.avg_blood_pressure_systolic and vitals.avg_blood_pressure_diastolic:
            ...
            bp_indicator = self._get_bp_indicator(
                ...
                vitals.avg_blood_pressure_diastolic
            )
            lines.append(
    ...
    def _generate_nutrition_section(
        ...
        if nutrition.avg_daily_calories:
            ...
            lines.append(f"Balanced Days: {nutrition.balanced_days}/{nutrition.total_days}")
        
        if detailed and has_data:
            if nutrition.avg_sodium_mg:
                sodium_status = self.EMOJI_WARNING if nutrition.avg_sodium_mg > 2300 else self.EMOJI_CHECK
                lines.append(f"Sodium: {nutrition.avg_sodium_mg:.0f} mg/day {sodium_status}")
            if nutrition.avg_sugar_g:
                sugar_status = self.EMOJI_WARNING if nutrition.avg_sugar_g > 50 else self.EMOJI_CHECK
    ...
    def _generate_medication_section(
        ...
        lines.append(f"Compliance: {meds.compliance_percentage:.0f}% {compliance_emoji}")
    ...
    def _generate_exercise_section(self, data: WeeklyHealthData) -> Optional[str]:
        ...
        lines.append(f"Weekly Goal: {goal_bar} {exercise.goal_completion_percentage:.0f}%")
    ...
    def _generate_alerts_section(self, data: WeeklyHealthData) -> Optional[str]:
        ...
        if data.medications.compliance_percentage < 80 and data.medications.total_medications > 0:
