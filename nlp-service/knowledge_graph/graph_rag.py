            ...
            for entity in self.primary_entities[:5]:
                ...
                if entity.related_entities:
                    for rel in entity.related_entities[:3]:
                        parts.append(f"  → {rel.get('relationship', '')}: {rel.get('entity', '')}")
...
class GraphRAGService:
    ...
    ENTITY_KEYWORDS = {
        NodeLabel.SYMPTOM.value: [
            "pain", "ache", "fatigue", "dizziness", "nausea",
            "shortness", "breath", "swelling", "palpitation",
        ],
        NodeLabel.CONDITION.value: [
            "disease", "syndrome", "disorder", "condition",
            "hypertension", "diabetes", "failure", "attack",
        ],
        NodeLabel.MEDICATION.value: [
            "drug", "medication", "medicine", "pill", "tablet",
            "dose", "prescription", "aspirin", "statin",
        ],
        NodeLabel.TREATMENT.value: [
            "treatment", "therapy", "surgery", "procedure",
            "intervention", "lifestyle", "diet", "exercise",
        ],
    }
        ...
        for record in query_result.records:
            ...
            if node_id:
                ...
                for neighbor in neighbors[:5]:
                    ...
                    for rel in rels:
                        if rel.end_node_id == neighbor.id or rel.start_node_id == neighbor.id:
                            rel_type = rel.type
                            break
    ...
    async def _semantic_graph_search(
        ...
        if "treat" in query_lower or "medication" in query_lower:
            ...
            results.extend(self._get_mock_results("treatments", NodeLabel.TREATMENT.value))
        
        if "risk" in query_lower or "factor" in query_lower:
            # User asking about risk factors
            results.extend(self._get_mock_results("risk factors", NodeLabel.RISK_FACTOR.value))
    ...
    def _get_mock_results(
        ...
        mock_data = {
            "chest pain": GraphSearchResult(
                ...
                related_entities=[
                    {"entity": "Coronary Artery Disease", "type": "Condition", "relationship": "INDICATES"},
                    {"entity": "Myocardial Infarction", "type": "Condition", "relationship": "INDICATES"},
                    {"entity": "Angina", "type": "Condition", "relationship": "INDICATES"},
                ],
                reasoning_path=[
            ...
            "hypertension": GraphSearchResult(
                ...
                related_entities=[
                    {"entity": "ACE Inhibitors", "type": "Medication", "relationship": "TREATS"},
                    {"entity": "Beta Blockers", "type": "Medication", "relationship": "TREATS"},
                    {"entity": "Stroke", "type": "Condition", "relationship": "RISK_FOR"},
                ],
                reasoning_path=[
            ...
            "shortness of breath": GraphSearchResult(
                ...
                related_entities=[
                    {"entity": "Heart Failure", "type": "Condition", "relationship": "INDICATES"},
                    {"entity": "COPD", "type": "Condition", "relationship": "INDICATES"},
                ],
                reasoning_path=[
