...
class DrugLabelQuerier:
    ...
    def _search_section(self, drug_name: str, section_field: str, limit: int = 5) -> List[str]:
        ...
        result = self.client.search_drug_label(query, limit=limit)
    ...
    def find_drug_interactions(self, drug_name: str, limit: int = 5) -> List[str]:
        """
        ...
        return self._search_section(drug_name, "drug_interactions", limit)

    def find_warnings(self, drug_name: str, limit: int = 5) -> List[str]:
        """
        Find boxed warnings and general warnings.
        """
        logger.info(f"Searching warnings for {drug_name}")
        boxed = self._search_section(drug_name, "boxed_warning", limit)
        general = self._search_section(drug_name, "warnings", limit)
        return boxed + general

    def find_adverse_reactions(self, drug_name: str, limit: int = 5) -> List[str]:
        """
        Find listed adverse reactions (side effects).
        """
        logger.info(f"Searching adverse reactions for {drug_name}")
        return self._search_section(drug_name, "adverse_reactions", limit)
    
    def find_contraindications(self, drug_name: str, limit: int = 5) -> List[str]:
        """
        Find contraindications (when NOT to use the drug).
        """
        logger.info(f"Searching contraindications for {drug_name}")
        return self._search_section(drug_name, "contraindications", limit)
