...
const CustomVideoPlayer = ({ src, poster, isExpanded, toggleExpand }: { src: string, poster?: string, isExpanded: boolean, toggleExpand: () => void }) => {
    ...
    return (
        <div 
            className={`relative group bg-black rounded-2xl overflow-hidden shadow-md transition-all duration-300 ${
                isExpanded ? 'fixed inset-0 z-[100] rounded-none' : 'w-full h-full'
            }`}
           ...
           {!isActive ? (
               ...
               <div className="flex gap-3 items-end">
                   ...
                   <button 
                       ...
                       className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-all shadow-lg shrink-0 ${
                           isLiveCoachActive 
