...
import * as recipeModule from './recipes';

// Get the raw recipe data from the module
const rawRecipeData = (recipeModule as any).default || recipeModule;

// Define the expected recipe structure for the application
interface Recipe {
  id: string;
  title: string;
  description: string;
...
const transformRecipe = (rawRecipe: any): { recipe: Recipe } => {
  // Extract steps from the process field by splitting on double newlines
  let steps: string[] = [];
  if (rawRecipe.process) {
    steps = rawRecipe.process
      .split(/\r?\n\s*\r?\n/) // Split on double newlines (paragraph breaks)
      .filter(step => step.trim() !== '') // Remove empty steps
      .map(step => step.replace(/^step \d+\s*/i, '').trim()); // Remove "step X" prefixes
  } else {
  ...
  const ingredients = rawRecipe.ingredients.map((ing: any) => {
    ...
    const measureMatch = measure.match(/^([\d./\s]+(?:\s*[½¼¾⅓⅔⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞\d]+)?)\s*(.*)/);
  ...
  return {
    recipe: {
      ...
      tags: [
        rawRecipe.category, 
        rawRecipe.area, 
        ...(rawRecipe.tags ? (Array.isArray(rawRecipe.tags) ? rawRecipe.tags : [rawRecipe.tags]) : []),
        rawRecipe.name.toLowerCase().replace(/\s+/g, '-')
      ].filter(tag => tag && tag !== 'null' && typeof tag === 'string'),
