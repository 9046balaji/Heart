...
# SECURITY: Strict multi-tenant mode flag
# When True, requests without active context will fail instead of using global instances
_strict_multi_tenant_mode: bool = False


def enable_strict_multi_tenant_mode():
    """
    Enable strict multi-tenant mode - REQUIRED for production deployments.
    
    When enabled, requests without an active context will fail rather than
    falling back to the global instance list, preventing cross-user data leakage.
    
    Example:
        # In your FastAPI startup
        from memori.integrations.openai_integration import enable_strict_multi_tenant_mode
        enable_strict_multi_tenant_mode()
    """
    global _strict_multi_tenant_mode
    _strict_multi_tenant_mode = True
    logger.warning(
        "Strict multi-tenant mode ENABLED. All requests must call "
        "set_active_memori_context() before making LLM calls."
    )


def disable_strict_multi_tenant_mode():
    """Disable strict multi-tenant mode (for testing or single-tenant apps)."""
    global _strict_multi_tenant_mode
    _strict_multi_tenant_mode = False
    logger.info("Strict multi-tenant mode disabled.")


def is_strict_multi_tenant_mode() -> bool:
    """Check if strict multi-tenant mode is enabled."""
    return _strict_multi_tenant_mode
...
class OpenAIInterceptor:
    ...
    def _inject_context_for_enabled_instances(cls, options, client_type):
        """Inject context for the active Memori instance (with security checks for multi-tenant mode)."""
        # Check if there's an active context (multi-tenant mode)
        active_memori = get_active_memori_context(require_valid=False)

        # SECURITY FIX: In strict mode, require explicit context to prevent cross-user data leakage
        if active_memori:
            memori_instances = [active_memori]
        elif _strict_multi_tenant_mode:
            # SECURITY: Do NOT fall back to global instances in strict mode
            logger.error(
                f"SECURITY: No active Memori context in strict multi-tenant mode for {client_type}. "
                "This request will NOT have memory context injected. "
                "Call set_active_memori_context(memori) before making LLM calls."
            )
            return options
        else:
            # Backward compatibility: Fall back to all instances (single-tenant only)
            memori_instances = _enabled_memori_instances
            if len(memori_instances) > 1:
                logger.warning(
                    f"Multiple Memori instances ({len(memori_instances)}) without active context. "
                    "Consider using set_active_memori_context() or enable_strict_multi_tenant_mode()."
                )
    ...
    def _record_conversation_for_enabled_instances(cls, options, response, client_type):
        """Record conversation for the active Memori instance (with security checks for multi-tenant mode)."""
        # Check if there's an active context (multi-tenant mode)
        active_memori = get_active_memori_context(require_valid=False)
        ...
        if active_memori:
            ...
            # SECURITY: Do NOT record to global instances in strict mode
            logger.error(
                f"SECURITY: No active Memori context in strict multi-tenant mode for {client_type}. "
            ...
            if len(memori_instances) > 1:
                logger.warning(
                    f"Recording to multiple Memori instances ({len(memori_instances)}) without active context. "
                    "This may cause data to be recorded to wrong user. "
                    "Consider using set_active_memori_context() or enable_strict_multi_tenant_mode()."
                )
