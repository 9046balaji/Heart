...
class Memori:
    ...
    def __init__(
        ...
        if provider_config:
            ...
            try:
                ...
                elif api_type == "openrouter" or (base_url and "openrouter" in base_url):
                    # Explicitly configured OpenRouter
                    self.provider_config = ProviderConfig.from_openrouter(
                        api_key=api_key or openai_api_key,
                        model=model,
                    )
                    logger.info("Using explicitly configured OpenRouter provider")
                elif api_type == "llama_local" or (base_url and ("localhost" in base_url or "127.0.0.1" in base_url)):
                    # Explicitly configured Llama Local
                    self.provider_config = ProviderConfig.from_llama_local(
                        base_url=base_url,
                        model=model,
                    )
                    logger.info("Using explicitly configured Llama Local provider")
                elif base_url:
            ...
            # Use environment-based provider detection (supports OpenRouter, Llama Local, OpenAI)
            try:
                from .providers import detect_provider_from_env

                self.provider_config = detect_provider_from_env()
                logger.info(
                    f"Using auto-detected provider from environment: {self.provider_config.api_type}"
                )
            except ImportError:
                logger.warning(
                    "ProviderConfig not available, using basic configuration"
    ...
    def search(self, query: str, user_id: str | None = None, limit: int = 5) -> list[dict[str, Any]]:
        ...
        # but retrieve_context uses self.user_id internally
        _ = user_id  # Acknowledged but uses instance user_id
    ...
    # =========================================================================
    # AbstractMemoriInterface Implementation (for MemoriRAGBridge compatibility)
    # =========================================================================
    ...
    def get_all_memories(
        ...
        Required by AbstractMemoriInterface for MemoriRAGBridge integration.

        Args:
            user_id: User identifier (uses self.user_id if not provided)
        ...
        try:
            ...
            for r in results:
                normalized.append({
                    ...
                    "content": r.get("content", r.get("searchable_content", r.get("summary", ""))),
                    "metadata": {
                        "category": r.get("category_primary", r.get("category", "general")),
    ...
    def add_memory(
        ...
        chat_id = self.record_conversation(
            user_input=content,
            ai_output="Memory recorded",
            metadata={
                "user_id": user_id,
                "type": "manual_memory",
                "source": "add_memory_method",
                **(metadata or {}),
            },
        )
        return {"id": chat_id, "memory_id": chat_id}
    ...
    def update_memory(
        ...
        """
        try:
            # Check if db_manager has update_memory method
            if hasattr(self.db_manager, "update_memory"):
                ...
                logger.warning(
                    f"update_memory not implemented in db_manager ({type(self.db_manager).__name__})"
                )
                return False
        except Exception as e:
    ...
    def delete_memory(self, memory_id: str) -> bool:
        ...
        """
        try:
            # Check if db_manager has delete_memory method
            if hasattr(self.db_manager, "delete_memory"):
                return self.db_manager.delete_memory(memory_id=memory_id)
            else:
                # Fallback: Log warning and return False
                logger.warning(
                    f"delete_memory not implemented in db_manager ({type(self.db_manager).__name__})"
                )
                return False
        except Exception as e:
