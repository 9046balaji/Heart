...
from core.config.compat import get_settings
...
class XAMPPDatabase:
    ...
    def __init__(self):
        self.write_pool: Optional[Any] = None  # aiomysql.Pool when available
        self.read_pool: Optional[Any] = None   # aiomysql.Pool when available
    ...
    def get_pool(self, operation: Literal["read", "write"] = "write") -> Any:
    ...
    async def fetch_all(self, query: str, params: tuple = None) -> List[Dict[str, Any]]:
        """
        Convenience method: Execute SELECT query and return all results as dicts.
        ...
        Example:
            results = await db.fetch_all(
                "SELECT * FROM vitals WHERE user_id = %s",
                (user_id,)
            )
        """
    ...
    async def fetch_one(self, query: str, params: tuple = None) -> Optional[Dict[str, Any]]:
        """
        Convenience method: Execute SELECT query and return single result as dict.
    ...
    async def execute_select(self, query: str, params: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
        Execute SELECT query with named parameters (supports :param_name syntax).
        
        This method is used by text_to_sql_tool.py which generates queries with
        parameterized placeholders like :user_id.
        
        Args:
            query: SQL SELECT query with :param_name placeholders
        ...
        Example:
            results = await db.execute_select(
                "SELECT * FROM vitals WHERE user_id = :user_id AND metric = :metric",
                {"user_id": "user123", "metric": "heart_rate"}
            )
        """
