...
def get_ollama_generator(model_name: str = "gemma3:1b"):
    ...
    """
    try:
        from ollama_generator import OllamaGenerator  # type: ignore
        return OllamaGenerator(model_name=model_name)
    except ImportError:
        # Fallback to core.llm.ollama_gateway
        try:
            from core.llm.ollama_gateway import OllamaGateway  # type: ignore
            return OllamaGateway(model_name=model_name)
        except ImportError:
            return None


async def get_rag_context(
    query: str, user_id: Optional[str] = None, top_k: int = 3
) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        from rag_api import get_rag_pipeline  # type: ignore
        pipeline = get_rag_pipeline()
    except ImportError:
        logger.warning("rag_api module not found, RAG context unavailable")
        return None
    ...
    return {
        ...
        "user_memory_context": result.to_dict()
        .get("sources", {})
        .get("memories", []),
